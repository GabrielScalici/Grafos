...
void imprime_ordem(PILHA *pilha){
    int x;
...
void dfs_grafo_d_l_visit(GRAFO_D_L *grafo, CELL *raiz, PILHA *pilha, char *cor){
    CELL *paux = grafo->vertices[raiz->V].lista_inicio;
    while(paux != NULL){
        if(cor[paux->V] != BLACK){
            cor[raiz->V] = GRAY;
            dfs_grafo_d_l_visit(grafo, paux, pilha, cor);
        }
        paux = paux->prox;
    }

    cor[raiz->V] = BLACK;
    empilhar(pilha, raiz->V);

    return;
}

void dsf_grafo_d_l(GRAFO_D_L *grafo){
    int i;
    char *cor = (char *)malloc(sizeof(char)*(grafo->num_vertices));
    PILHA *pilha = criar_pilha();
    ...
    for(i=0;i<grafo->num_vertices;i++){
        if(cor[i] == WHITE){
            cor[i] = GRAY;
            CELL *paux = grafo->vertices[i].lista_inicio;
            while(paux!=NULL){
                if(cor[paux->V] != BLACK){
                    cor[paux->V] = GRAY;
                    dfs_grafo_d_l_visit(grafo,paux,pilha,cor);
